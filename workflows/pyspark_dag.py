...
PROJECT_ID = "bhb-retail-data-project"
REGION = "us-east1"
CLUSTER_NAME = "my-demo-cluster"
COMPOSER_BUCKET = "us-central1-demo-composer-ace5e4d3-bucket"
...
# define the dag
with DAG(
    dag_id="pyspark_dataproc_etl_dag",
    schedule_interval=None,
    description="DAG to run PySpark jobs on Dataproc and manage cluster lifecycle",
    default_args=ARGS,
    tags=["pyspark", "dataproc", "etl", "retail"]
) as dag:
    ...
    pyspark_task_1 = DataprocSubmitJobOperator(
        task_id="retailer_mysql_to_landing",
        job=PYSPARK_JOB_1,
        region=REGION,
        project_id=PROJECT_ID
    )
    ...
    pyspark_task_2 = DataprocSubmitJobOperator(
        task_id="supplier_mysql_to_landing",
        job=PYSPARK_JOB_2,
        region=REGION,
        project_id=PROJECT_ID
    )
    ...
    pyspark_task_3 = DataprocSubmitJobOperator(
        task_id="customer_reviews_api_to_landing",
        job=PYSPARK_JOB_3,
        region=REGION,
        project_id=PROJECT_ID
    )
